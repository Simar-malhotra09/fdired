...
FileTypeHandler file_type_handlers[] = {
    {FT_PDF, "open"},
    {FT_IMAGE, "open"},
    {FT_ASCII, "nvim"},
};
...
SearchResult parse_single_output(char *line, char *pattern,
                                 InputArgs input_args, AVAILABLE_CMDS cmd);
 ...
 * UtilityOutput */
void render(viewport *v, UtilityOutput *out, char *pattern,
            AVAILABLE_CMDS cmd_type, PATH_STATE path_state);

/* match the file into FileType to know how to open it */
FileType match_file_type(char *file_path);
const char *get_handler_for_file(FileType file_type);
...
FileType match_file_type(char *file_path) { return FT_ASCII; }
const char *get_handler_for_file(FileType file_type) {
  for (size_t i = 0;
